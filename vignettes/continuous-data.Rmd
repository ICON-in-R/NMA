...
data_type <- "conts_data"     # which type of data to use
...
contsData <-
  read.csv(paste0(file_name, "/conts_data_test.csv"),
           header = TRUE,
           as.is = TRUE)
```
...
```{r echo=FALSE}
knitr::kable(contsData)
```

More information about these data is available in the help documentation which can be accessed with e.g. `help(contsData)`.
